...
import { scenarios, type ModuleScenario } from '@/lib/lab/modules-data'
...
export function ModulesViz() {
  ...
  const scenario: ModuleScenario = scenarios[scenarioIndex]
      ...
      {step.vars.length > 0 && (
        <div className="border border-stroke">
          <div className="px-4 py-2.5 border-b border-stroke flex items-center justify-between">
            <span className="font-mono text-[10px] text-lab-blue tracking-[0.16em] uppercase">Variables</span>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 divide-y divide-stroke sm:divide-y-0 sm:divide-x">
            {step.vars.map((v) => (
              <div key={v.label} className="flex items-baseline justify-between gap-3 px-4 py-2">
                <span className="font-mono text-[10px] text-muted tracking-[0.1em] uppercase">{v.label}</span>
