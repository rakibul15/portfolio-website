...
import { scenarios, type MemoryScenario } from '@/lib/lab/memory-gc-data'
...
export function MemoryGCViz() {
  ...
  const scenario: MemoryScenario = scenarios[scenarioIndex]
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {step.vars.length > 0 && (
        <div className="border border-stroke">
          <div className="px-4 py-2.5 border-b border-stroke flex items-center justify-between">
            <span className="font-mono text-[10px] text-lab-amber tracking-[0.16em] uppercase">Variables</span>
            <span className="font-mono text-[10px] text-faint tracking-[0.06em]">{step.vars.length}</span>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 divide-y divide-stroke sm:divide-y-0 sm:divide-x">
            {step.vars.map((v) => (
              <div key={v.label} className="flex items-baseline justify-between gap-3 px-4 py-2">
                <span className="font-mono text-[10px] text-muted tracking-[0.1em] uppercase whitespace-nowrap">{v.label}</span>
                <span className="font-mono text-[11px] text-ink text-right truncate">{v.value}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="hidden lg:block">
        <Group orientation="horizontal" className="min-h-[520px]">
